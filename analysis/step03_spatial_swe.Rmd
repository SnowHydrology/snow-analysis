...
There are multiple ways of accessing the data, but for expediency's sake, I downloaded the water year 2021 NetCDF from their file server: <https://climate.arizona.edu/data/UA_SWE/WYData_4km/UA_SWE_Depth_WY2021.nc>
...
We can also get some information about the data
...
There we can the basin outline for 06187915 and simulated SWE from `r time(plot_swe)`.
...
There are some obvious major differences (SWE magnitude, snow cover duration, snow off date, etc.), which we can explore at a later time. Right now I'll note that the UofA SWE product assimilates SNOTEL observed SWE and differences may arise from point vs. basin elevational differences, the assimilation protocol of the product, and the snowmelt model used to create it. (And, this is only one location so we shouldn't infer too much about model performance.)
